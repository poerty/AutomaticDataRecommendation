...
class MediaEditorExample extends React.Component {
  constructor(props) {
    ...
    this.sendRequest = () => {
      ...
      let temp=convertToRaw(content).blocks[0].text;
      console.log(temp);
  ...
  _confirmMedia(e) {
    ...
    const contentStateWithEntity = contentState.createEntity(
      urlType,
      'IMMUTABLE',
      {src: 'https://raw.githubusercontent.com/facebook/draft-js/master/examples/draft-0-10-0/media/' + urlValue}
    );
  ...
  render() {
    ...
    for(let news in this.state.newsList){
      newsLayer.push(
        <div className="cssZoom">
          <iframe
            src={this.state.newsList[news]['0']}
            frameborder="0"
            />
        </div>);
    }
    if(this.state.newsList=="fail"){
      newsLayer=[];
    }
    let zoomBoxStyle={
      width: 400*this.state.newsList.length
    };
    ...
    return (
      <div style={styles.root}>
        ...
        <div className="scrollBox">
          <div style={zoomBoxStyle} className="zoomBox">
            {newsLayer}
          </div>
        </div>
      </div>
